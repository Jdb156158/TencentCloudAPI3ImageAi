...
class JunVisionTool: NSObject {
    ...
    typealias faceFuncCallBack = (_ rect:CGRect) -> Void
    typealias faceFeatureCallBack = (_ bigRectArr: [CGPoint]?) -> Void

    // 特征点识别
    @objc func faceFeature(faceImage:UIImage,cleanView:UIView,completeBack: @escaping faceFeatureCallBack) {
        visionDetectImage(type: .feature, image: faceImage) {(bigRects, smallRects) in
            //1. 获取识别结果
            guard let faceArr = smallRects as? [FaceFeatureModel] else { return }
            
            //2. 遍历识别的人脸
            DispatchQueue.main.async {
                let view = FaceFeatureView(frame: CGRect(x: 0, y: 0, width: cleanView.frame.width, height: cleanView.frame.height), faceArr: faceArr)
                cleanView.addSubview(view)
            }
            
            //
            let medianLine = faceArr.first?.medianLine

            let point = medianLine!.normalizedPoints
    ...
    // 静态人脸
    @objc func staticFaceFunc(faceImage:UIImage,cleanView:UIView,completeBack: @escaping faceFuncCallBack) {
        visionTool.visionDetectImage(type: .staticFace, image: faceImage) { (bigRects, smallRects)  in
            //2. 识别到的大区域
            guard let rectArr = bigRects else { return }
            for textRect in rectArr{
                DispatchQueue.main.async {
                    cleanView.addSubview(viewTool.addRectangleView(rect: textRect))
                    completeBack(textRect)
                }
            }
        }
    }
